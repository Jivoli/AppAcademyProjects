...
def every?(arr, &prc)
    ...
    return true if counter == arr.length
    return false
end
...
def filter_out!(arr, &prc1)
    arr.reject! { |ele| prc1.call(ele)}
end
...
def proctition(arr, &prc)
    ...
    arr_true = []
    arr.each do |ele|
        arr_false << ele if !prc.call(ele)
        arr_true << ele if prc.call(ele)
    end
    arr_true + arr_false
end
...
def hash_has_value(ele,hash)
    hash.each do |k, v|
        return true if v.include?(ele)
    end
    return false
...
def proctition_platinum(arr, *procs)
     ...
     arr.each do |ele|
        procs.each.with_index do |prc, idx|
             ...
             hash[idx + 1] << ele if prc.call(ele) && !hash_has_value(ele, hash)
...
#             if prc.call(ele)
#                 hash[idx + 1] << ele
#                 arr.reject! { |element| element == ele}
#             end
#         end
#     end
#     return hash
...
def procipher(sent, hash)
    final_sentence = sent.split(" ")
    aldone_idx_words = []
    words_to_join = []

    final_sentence.each.with_index do |word,idx|
        hash.each do |k, v|
            if k.call(word) && !aldone_idx_words.include?(idx)
                aldone_idx_words << idx
                words_to_join << v.call(word)
            elsif k.call(word) && aldone_idx_words.include?(idx)
                modified_word = words_to_join[-1]
                words_to_join[-1] = v.call(modified_word)
            end
        end
        words_to_join << word if !aldone_idx_words.include?(idx)
    end

    return words_to_join.join(" ")
end
...
def picky_procipher(sent, hash)
    ...
    final_sentence.each.with_index do |word,idx|
        hash.each do |k, v|
            if k.call(word) && !aldone_words.include?(word)
                aldone_words << word
                words_to_join << v.call(word)
            end
        end
        words_to_join << word if !aldone_words.include?(word)
    end
